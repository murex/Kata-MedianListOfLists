# Getting Started with Median of a List of Lists (with no concatenation) kata in C++
...
1. When executed on Windows and macOS, our script will, by default, generate the solution files ***Kata-MedianListOfLists.sln*** for **'Visual Studio 2017'** & ***Kata-MedianListOfLists.xcodeproj*** for **'Xcode'** respectively. 
...
```shell
    Start 1: ctest-Kata-MedianListOfLists
1/1 Test #1: ctest-Kata-MedianListOfLists ...........   Passed    1.21 sec
...
1. For *Windows*: The **'Visual Studio 2017'** solution file `build` / `Kata-MedianListOfLists.sln`
1. For *macOS*: The **'Xcode'** project file `build` / `Kata-MedianListOfLists.xcodeproj`

### Using IDEs 

The CMake file we provided generates the target 'Kata-MedianListOfLists-test' that defines the test location (i.e. [MedianListOfListsTest.cpp](./test/MedianListOfListsTest.cpp)). 

#### Visual Studio 2017

You can open the project in VS 2017, by simply running the solution file `Kata-MedianListOfLists` / `cpp` / `build` / `Kata-MedianListOfLists.sln`

After loading the project into your IDE:

1. Make sure the 'Kata-MedianListOfLists-test' project is set as the StartUp Project then run the project.
2. Run the project (in VS use '**F5**' or click on '**Local Window Debugger**' in the toolbar). The following output should be generated: 
    ```shell
    Running main() from .../Kata-MedianListOfLists/cpp/build/_deps/googletest-src/googletest/src/gtest_main.cc
...
1. Start CLion and open the project from `Kata-MedianListOfLists` / `cpp` folder. 
1. CLion should automatically build the CMake file. 
1. Run the test file [MedianListOfListsTest.cpp](./test/MedianListOfListsTest.cpp).
