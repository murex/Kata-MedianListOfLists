...
- Generate the solution file ***Kata-MedianListOfLists.sln*** for **Visual Studio 2017** on Windows,
...
> ***Reminder***: the commands below should be run from [Kata-MedianListOfLists/cpp]() directory
...
> ***Reminder***: the command below should be run from [Kata-MedianListOfLists/cpp]() directory
...
`Kata-MedianListOfLists` / `cpp` / `build` / `Kata-MedianListOfLists.sln`

After loading the solution:

1. Make sure the 'Kata-MedianListOfLists-test' project is set as the Start-up Project.
...
`File` > `Open` > `Kata-MedianListOfLists` > `cpp`

CLion should automatically build the CMake file.

Run the test file [MedianListOfListsTest.cpp](./test/MedianListOfListsTest.cpp)
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-MedianListOfLists` / `cpp`
...
Open Xcode, choose `Open a project or file`, navigate to `Kata-MedianListOfLists` / `cpp` / `build` / `Kata-MedianListOfLists.xcodeproj`
then click `Open`.

Once the project is opened in Xcode, select `Product` / `Scheme` / `Kata-MedianListOfLists-test` to set it as the current active scheme.
