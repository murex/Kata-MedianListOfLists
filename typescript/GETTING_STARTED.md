# Getting Started with Median of a List of Lists (with no concatenation) kata in Typescript
...
`File` > `Open` > `Kata-MedianListOfLists` > `typescript`
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-MedianListOfLists` / `typescript`
