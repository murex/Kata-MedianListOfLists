# Getting Started with Median of a List of Lists (with no concatenation) kata in Python
...
> ***Reminder***: the commands below should be run from [Kata-MedianListOfLists/python](.) directory
...
`File` > `Open` > `Kata-MedianListOfLists` > `python`
...
> ***Reminder***: the command below should be run from [Kata-MedianListOfLists/python](.) directory
...
Choose `Open Folder`, navigate to `Kata-MedianListOfLists` / `python` then click `Select Folder`.
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-MedianListOfLists` / `python`
