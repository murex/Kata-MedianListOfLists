# Getting Started with Median of a List of Lists (with no concatenation) kata in Go
...
> ***Reminder***: the commands below should be run from [Kata-MedianListOfLists/go]() directory
...
> ***Reminder***: the command below should be run from [Kata-MedianListOfLists/go]() directory
...
`File` > `Open` > `Kata-MedianListOfLists` > `go`

Run all the tests in [median_list_of_lists_test.go](./src/median_list_of_lists/median_list_of_lists_test.go).
...
Open Visual Studio Code, choose `Open Folder`, navigate to `Kata-MedianListOfLists` / `go`
